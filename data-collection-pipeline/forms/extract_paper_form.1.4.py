...
Usage:
    python3 extract_paper_form.1.4.py <pdf_path> <fieldmap.json> <schema.json> <output_basename> [--model <checkbox_model.pkl>]
    python3 extract_paper_form.1.4.py --batch <folder_of_pdfs> <fieldmap.json> <schema.json> <output_basename> [--model <checkbox_model.pkl>]
    python3 extract_paper_form.1.4.py --review <pdf_path> <fieldmap.json> <schema.json> <out_dir> [--model <checkbox_model.pkl>]
    python3 extract_paper_form.1.4.py --review --batch <folder_of_pdfs> <fieldmap.json> <schema.json> <out_dir> [--model <checkbox_model.pkl>]
    python3 extract_paper_form.1.4.py --ingest <corrected_manifest.csv> <labels_dir>
"""
...
def extract_from_images(page_images, fieldmap, schema, source_channel="paper_scan", checkbox_model=None):
    ...
    for page_num in range(1, n_pages + 1):
        ...
        for f, cb_px in zip(item_fields, cb_located):
            ...
            low_confidence = confidence is not None and confidence < ML_CONFIDENCE_THRESHOLD
            line_items.append({
...
def extract_from_pdf(pdf_path, fieldmap, schema, checkbox_model=None):
    page_images = pdf_to_page_images(pdf_path)
    return extract_from_images(page_images, fieldmap, schema, checkbox_model=checkbox_model)


def process_one(pdf_path, fieldmap, schema, form_id, checkbox_model=None):
    submission, line_items = extract_from_pdf(pdf_path, fieldmap, schema, checkbox_model=checkbox_model)
...
def export_review_crops(pdf_path, fieldmap, schema, form_id, out_dir, checkbox_model=None):
    country = fieldmap["country"]
    mark_style = fieldmap.get("mark_style", "check")
    item_lookup = build_item_lookup(schema, country)
...
def main():
    ...
    # --model <path> is accepted anywhere on the command line and pulled out
    # first so it doesn't disturb the existing positional argument handling
    # below; omitting it reproduces v1.3 behavior exactly.
    model_path = None
    if "--model" in args:
        i = args.index("--model")
        model_path = args[i + 1]
    ...
    if args and args[0] == "--review":
        ...
        for i, pdf_path in enumerate(pdf_paths, start=1):
            ...
            export_review_crops(pdf_path, fieldmap, schema, form_id, out_dir, checkbox_model=checkbox_model)
        return
    ...
    for i, pdf_path in enumerate(pdf_paths, start=1):
        ...
        submission, line_items = process_one(pdf_path, fieldmap, schema, form_id, checkbox_model=checkbox_model)
