"""
v1.1 changes from build_review_tool.1.0.py:
  - Item checkbox/price cards now show a line number badge (#1, #2, ...) --
    checkbox #N and price #N are always the same item, so an operator can say
    "line 7" and mean one specific row in both sections.
  - New bulk-confirm bar: "Are all other lines blank?" -- clicking Yes marks
    every line the pipeline read as blank (checkbox blank + price blank) as
    confirmed-blank in one action, instead of an operator re-typing "blank"
    into dozens of already-correct fields one at a time. If some of those
    lines are NOT actually blank, the operator lists their line numbers as
    exceptions instead: everything else gets bulk-confirmed, and the listed
    lines are left untouched and visually flagged for individual correction
    (and exported with flagged=True, so a pipeline false-negative -- a real
    mark the extractor missed entirely -- is easy to find later).

Usage unchanged: python3 build_review_tool.1.1.py <review_manifest.csv> <output.html>
"""
...
LINE_NUMBERED_TYPES = ("checkbox", "price")  # these two share the same line numbering
    ...
    for ft in FIELD_TYPE_ORDER:
        ...
        if ft == "checkbox":
            ...
            <div class="bulk-bar">
              ...
              <button class="yes" onclick="confirmAllBlank([])">Yes, confirm all</button>
              <span class="or">or list the line number(s) that are NOT actually blank:</span>
              <input id="exceptionLines" type="text" placeholder="e.g. 3, 9" style="width:120px">
              <button class="flag" onclick="confirmAllBlank(parseExceptions())">Apply</button>
            </div>"""
        sections.append(f"""
        <section>
...
function parseExceptions() {{
  const raw = document.getElementById('exceptionLines').value || '';
  return raw.split(/[,\\s]+/).map(s => s.trim()).filter(Boolean).map(Number).filter(n => !isNaN(n));
}}

function confirmAllBlank(exceptions) {{
  const exceptSet = new Set(exceptions);
  const checkboxCards = document.querySelectorAll('.card[data-field_type="checkbox"]');
  ...
  checkboxCards.forEach(cbCard => {{
    ...
    if (!wasBlank) return;  // pipeline already found a mark here -- not part of "the blank lines"
    const priceCard = document.querySelector(
